...
// val dataRDD: RDD[Triple] = NTripleReader.load(spark, conf.getString("slipo.data.input")).persist()
  val dataRDD: RDD[Triple] = loadNTriple("/home/rajjat/IAIS/Projects/SANSA_POI/sansa-poi/target/CORE/data/dataPOI_10000.nt")
  ...
  var poiCoordinates: RDD[(Long, CoordinatePOI)] = this.getPOICoordinates.persist()
  ...
  def broadCastJoin(poiAllCategories: RDD[(Long, (Categories, Double))], poiCoordinates: RDD[(Long, CoordinatePOI)]): RDD[Poi] = {
  ...
  def filterJoin(poiAllCategories: scala.collection.Map[Long, (Categories, Double)], poiCoordinates: RDD[(Long, CoordinatePOI)]): RDD[Poi] = {
